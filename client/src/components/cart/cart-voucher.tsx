  ...
  const { data } = useFindManyCouponQuery({
    variables: {
      where: {
        ...
        website: {
          slug: {
            equals: settings?.websiteSlug,
          },
        },
      },
    },
    skip: !settings?.websiteSlug,
  });
  useOnClickOutside(voucherRef, () => setToggleVoucher(false));
  function handleOnSubmit(e: React.SyntheticEvent) {
    e.preventDefault();
    const coupon = data?.findManyCoupon.find((coupon) => coupon.code === code);
    if (coupon) {
      setCoupon(coupon);
    }
    setToggleVoucher(!toggleVoucher);
  }
  return (
    <div ref={voucherRef} className="text-center mt-2 mb-5">
      <AnimatePresence initial={false}>
        {toggleVoucher ? (
          ...
          >
            {coupon ? (
              <>{coupon?.couponType === 'amount_off' ? `$${coupon.amount_off} ` : `${coupon.percent_off}%`} off</>
            ) : (
              <>{t('text-have-coupon')}</>
            )}
          </motion.button>
