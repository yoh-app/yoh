...
const ForgetPassword = () => {
  ...
  const [verifyToken, { loading: verifying }] = useVerifyForgetPasswordTokenMutation();
  ...
  return (
    <div className="py-6 px-5 sm:p-8 bg-light w-screen md:max-w-md h-screen md:h-auto flex flex-col justify-center">
      ...
      {errorMsg && (
        <Alert variant="error" message={errorMsg} className="mb-6" closeable={true} onClose={() => setErrorMsg('')} />
      )}
      {!verifiedEmail && <EnterEmailView loading={loading} onSubmit={handleEmailSubmit} />}
