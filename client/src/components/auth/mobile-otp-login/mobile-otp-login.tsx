...
const MobileOtpLogin: React.FC<Props> = ({ type, onVerify, userId, className }) => {
  ...
  //       setIsContactExist(type !== OtpType.LOGIN ? true : data?.sendOtpCode?.is_contact_exist!);
  ...
  // const [verifyOtpCode, { loading: otpVerifyLoading }] = useVerifyOtpCodeMutation({
  //   onCompleted: (data) => {
      ...
      {!showOtpInput ? (
        <div>
          <div className="flex items-center">
            ...
            {/* <Button loading={loading} disabled={loading} onClick={onSendCodeSubmission} className="!rounded-s-none">
              {t('text-send-code')}
            </Button> */}
          </div>
          {otpSendError && (
        ...
        <div className="space-y-5 border border-gray-200 rounded p-5">
          ...
          <div className="grid grid-cols-2 gap-5">
            ...
            {/* <Button
              loading={otpLoginLoading || updateContactLoading || otpVerifyLoading}
              disabled={updateContactLoading || otpLoginLoading || otpVerifyLoading || !otp}
              onClick={onSubmit}
            >
              {t('text-verify-code')}
            </Button> */}
