...
const RelatedProducts = dynamic(() => import('./product-details/related-products'));
            ...
            <div className="w-full">
              ...
              <div className={cn({ 'mt-4': !isEmpty(variations) })}>
                {quantity! > 0 ? (
                  ...
                  <div className="bg-red-500 rounded text-sm text-light px-3 py-2">{t('text-out-stock')}</div>
                )}
              </div>
            </div>
          </div>
          ...
          <div className="product-gallery h-full relative">
            ...
            {!!gallery?.length ? (
              ...
              <div className="w-full flex items-center justify-center">
                <Image src={image?.original ?? '/product-placeholder.svg'} alt={name!} width={450} height={450} />
              </div>
            )}
          </div>
        </div>
              ...
              <div className="w-full">
                ...
                <div className="mt-4 md:mt-6 flex flex-col lg:flex-row items-center">
                  ...
                  {quantity! > 0 ? (
                    ...
                    <div className="text-base text-red-500 whitespace-nowrap ms-7">{t('text-out-stock')}</div>
                  )}
                </div>
              </div>

              {!!categories?.length && (
                <ProductCategories categories={categories} basePath={`/${type?.slug}`} onClose={closeModal} />
              )}
