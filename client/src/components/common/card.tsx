...
const Card: React.FC<Props> = ({ item, variant = 'circle', size = 'small', effectActive = false, href }) => {
  const { name, image, description } = item ?? {};
  const imageSize: any = (size === 'small' && 100) || (size === 'medium' && 198);
  ...
  return (
    <Link href={href} className="group flex justify-center text-center flex-col">
      ...
      <Text variant="heading" className="capitalize truncate text-xs">
        {name}
      </Text>
      <Text className="capitalize truncate text-xs">{description}</Text>
    </Link>
  );
};
