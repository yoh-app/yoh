...
const AuthorizedMenu = dynamic(() => import('@components/layout/navbar/authorized-menu'), { ssr: false });
...
const NavbarWithSearch: React.FC<Props> = ({ hideTypeMenu = false }) => {
  ...
  return (
    <header ref={navbarRef} className="site-header-with-search h-14 md:h-16 lg:h-auto">
      <nav
        className={cn(
          ...
          {
            'fixed bg-light lg:bg-transparent lg:absolute': !displayHeaderSearch,
            'is-sticky fixed bg-light shadow-sm': displayHeaderSearch,
          },
        )}
      >
        {displayMobileSearch ? (
          ...
          <>
            ...
            {!hideTypeMenu && <ProductTypeMenu className="ms-10 me-auto hidden xl:block" />}
            <div className="hidden lg:block w-full">
              <div
                className={cn('w-full xl:w-11/12 2xl:w-10/12 mx-auto px-10 overflow-hidden', {
