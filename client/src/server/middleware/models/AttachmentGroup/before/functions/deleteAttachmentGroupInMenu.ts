...
const deleteattachmentGroupInMenu = {
  mutateResult: false,
  run: async (root, args, context, info) => {
    try {
      const { prisma, params } = context;
      const attachmentGroup = await prisma.attachmentGroup.findUnique({
        where: {
          id: args?.where?.id,
        },
        include: {
      ...
      await Promise.all(attachmentGroup.organization.pages.map(async (page) => {
...
export default deleteattachmentGroupInMenu;
