    ...
    try {
      if (
        ...
        if (process.env.NODE_ENV === 'production') {
          fetch(`https://${video.website.slug}.${context?.domain}/api/revalidate/video/${video.id}`).catch((err) => console.log(err))
          ...
          fetch(`http://www.awkns.local:3003/api/revalidate/video/${video.id}`).catch((err) => console.log(err))
        }
