      ...
      <Box sx={{
        ...
        <div>
          <Grid container>
            <Grid xs={12} md={6} item>
              <div
                onClick={() => {
                  window.location.assign(`/${router.locale}/admin/Website/Website/DesignPage?id=${indexPage?.id}`)
                }}
            ...
            <Grid xs={12} md={6} item>
              <div
                onClick={() => {
                  window.location.assign(`/${router.locale}/admin/Website/Website/Product/?view=${createdProduct?.id}`)
                }}
                style={{
                  ...gridStyle,
        ...
        {/* <div className="text-center mt-8">
          <SubmitButton variant="contained" onClick={() => {
            if (item === 'design') {
              window.location.assign(`/${router.locale}/admin/Website/Website/DesignPage?id=${indexPage?.id}`)
              // router.push(`/admin/Website/Website/DesignPage?id=${indexPage?.id}`)
            } else if (item === 'info') {
              window.location.assign(`/${router.locale}/admin/Website/Website/Product/?view=${createdProduct?.id}`)
              // router.push(`/admin/Website/Website/Product/?view=${createdProduct?.id}`)
            }
          }} disabled={!item}>
...
const PrismaTable: React.FC<ModelTableProps> = ({ children, websiteData, permissionData, language, model, ui, ...rest }) => {
  ...
  // const { data: stripeAccountStatusData } = useStripeAccountStatusQuery({
  //   skip: permissionData?.admin !== 'Website' || model !== 'Product'
  // });
