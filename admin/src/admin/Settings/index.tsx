...
import { usePermissionQuery, useGetAdminSettingsQuery } from 'generated';
...
export const Settings: React.FC<{
  ...
  const Admin = permissionData?.permission?.Admin;
  const Version = permissionData?.permission?.Version;
  const admin = permissionData?.permission?.admin;
  ...
  const models = data?.getAdminSettings.models ?? [];
  ...
  if (dataRef.current !== models && models.length > 0 && currentModel) {
    ...
    setCurrentModel(models.find((model) => model.name === currentModel.name));
  }
            ...
            <div className="w-full" style={{ marginBottom: '20px' }}>
              {currentModel && (
                <Select
                  ...
                  onChange={(option: any) => setCurrentModel(models.find((model) => model.name === option.name))}
