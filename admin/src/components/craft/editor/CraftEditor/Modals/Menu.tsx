...
import NewPagePrismaTable from './NewPagePrismaTable'
...
const PageSelectView = ({ closeModal, pages, addNew, column }) => {
  ...
  return (
    <div>
      ...
      <div className="text-center mt-8">
        <SubmitButton
          style={{ background: selected ? 'black' : 'lightgrey' }}
          variant="contained"
          disabled={!selected}
          onClick={async () => {
...
const NewPage = ({ closeModal, column, goBack }) => {
  ...
  const { setNextPage, setView } = useContext(CraftContext)
  const { board } = useSelector((state) => state.menu);
  const [createPage] = useMutation(gql`mutation createOnePage($data: PageCreateInput!) {
    createOnePage(data: $data) {
      id
...
export default function Menu({ refetchWebsite, setNextPage, setDesignView }) {
  ...
  const renderView = () => {
    switch (view) {
      ...
      case 'new_page':
        return <NewPagePrismaTable
          data={{}}
          setView={setView}
          onCancel={() => setView(null)}
          onSave={(data) => {
            dispatch(addTask({ card: data, board, columnId: column.id }));
            setNextPage(`/admin/Website/Website/DesignPage?id=${data?.id}`)
