...
const deletenftGroupInMenu = {
  mutateResult: false,
  run: async (root, args, context, info) => {
    try {
      const { prisma, params } = context;
      const nftGroup = await prisma.nftGroup.findUnique({
        where: {
          id: args?.where?.id,
        },
        include: {
      ...
      await Promise.all(nftGroup.organization.pages.map(async (page) => {
...
export default deletenftGroupInMenu;
