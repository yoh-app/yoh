...
const deletePageCollectionInMenu = {
  mutateResult: false,
  run: async (root, args, context, info) => {
    try {
      const { prisma, params } = context;
      const pageCollection = await prisma.pageCollection.findUnique({
        where: {
          id: args?.where?.id,
        },
        include: {
      ...
      await Promise.all(pageCollection.website.pages.map(async (page) => {
...
export default deletePageCollectionInMenu;
